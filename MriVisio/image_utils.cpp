...
image_t image_utils::lightening(image_t image)
{
	float kernel[9];
	kernel[0] = -0.1;
	kernel[1] = 0.2;
...
image_t image_utils::contour_to_image(contour_container conts, CvSize size, int depth, int channels)
{
	...
	while(!conts.is_end())
		cvDrawContours(dst, conts.get(), CV_RGB(255, 216, 0), CV_RGB(0, 0, 250), 0, 1, 8); // рисуем контур
...
image_t image_utils::contour_to_image(contour cont, CvSize size, int depth, int channels)
{
	image_t dst = image_utils::create(size, depth, channels);
	cvZero(dst);
	cvDrawContours(dst, cont, CV_RGB(255, 216, 0), CV_RGB(0, 0, 250), 0, 1, 8); // рисуем контур
	return dst;
}
...
contour image_utils::find_max_contour(contour_container cont)
{
	...
	while (!cont.is_end())
	{
		...
		auto rect = cvBoundingRect(current);
...
image_t image_utils::smooth(image_t image)
{
	image_t dst = image_utils::clone(image);
	cvSmooth(image, dst, CV_BLUR, 10, 10);
	return dst;
}
